...
URL_RECAP = f"{BASE_BLD}/getBrRecapTitleInfo"
URL_TITLE = f"{BASE_BLD}/getBrTitleInfo"
URL_EXPOS = f"{BASE_BLD}/getBrExposPubuseAreaInfo"
URL_FLOOR = f"{BASE_BLD}/getBrFlrOulnInfo"
URL_ATCH  = f"{BASE_BLD}/getBrAtchJibunInfo"

EXPOS_MATCH    = "MATCH"
EXPOS_CONFLICT = "CONFLICT"
EXPOS_NONE     = "NONE"
...
# ★ PK 직접 조회: 동 단위로 조회하므로 건수가 적어 2000건 캡 문제 없음
...
def render_expos_card(grp, is_missing_area, conflict=False):
    ...
    with st.container(border=True):
        if conflict:
            st.warning(f"⚠️ 동 체계 불일치 — API 등록 동명: **{safe_val(mr.get('dongNm'))}** (호수·지번 기준 참고용)")
        st.markdown(f"#### 🚪 {full_nm}")
        st.caption("🔒 소유자 정보는 개인정보 보호로 제공되지 않습니다.")
        ...
        with c1:
            ...
            info_df = pd.DataFrame({
                "항목": ["주용도", "기타용도", "구조", "해당 층"],
                "내용": [
                    safe_val(mr.get("mainPurpsCdNm")),
                    safe_val(mr.get("etcPurps")),
                    safe_val(mr.get("strctCdNm")),
                    safe_val(mr.get("flrNoNm")),
                ]
            })
        ...
        with c2:
            ...
            if is_missing_area:
                ...
                g_str = f"{g_area:,.2f} ㎡"
                t_str = f"{t_area:,.2f} ㎡"
                p_str = f"약 {t_area / 3.3058:.1f} 평"

            area_df = pd.DataFrame({
                "구분": ["전용면적", "공용면적", "계약면적(합계)", "평형 환산"],
                "면적": [j_str, g_str, t_str, p_str],
            })
...
def get_building_ledger(sgg_cd, bjdong_cd, plat_gb, bun, ji, target_dong="", target_ho=""):
    ...
    # ★ 핵심 수정: skip_dong_check=True 시 dongNm 검증 생략 (PK 직접 조회 시 사용)
    def analyze_expos(df_src, source_label, skip_dong_check=False):
        ...
        # ★ PK 직접 조회는 이미 해당 동이 보장 → dongNm 검증 생략하고 바로 MATCH
        if skip_dong_check:
            df_ho = df_ho.drop(columns=["_jibun_key"], errors="ignore")
            if "area" not in df_ho.columns:
                df_ho["area"] = "0"
                is_missing_area = True
            expos_debug_log.append({"판정": "MATCH(PK보장)", "이유": "mgmBldrgstPk 직접조회이므로 dongNm 검증 생략"})
        ...
        expos_debug_log.append({
            "판정": "CONFLICT",
            "이유": f"호수 일치하나 동명 불일치 (입력:{target_dong} vs API:{api_dong})",
        })
        df_ho = df_ho.drop(columns=["_jibun_key"], errors="ignore")
        if "area" not in df_ho.columns:
            df_ho["area"] = "0"
    ...
    if target_ho:
        # ★ 1단계: target_pks로 mgmBldrgstPk 직접 조회 (동 단위 조회 → 2000건 캡 문제 없음)
        if target_pks:
            for pk in target_pks:
                ...
                if not items:
                    continue
                # ★ skip_dong_check=True: PK가 표제부에서 해당 동으로 확인됐으므로 dongNm 재검증 불필요
                m_df, c_df = analyze_expos(pd.DataFrame(items), f"pk={pk}", skip_dong_check=True)
                if not m_df.empty:
                    df_expos     = m_df
        ...
        # ★ 2단계: PK 조회로 MATCH 못 찾으면 bun/ji 전체 폴백
        if expos_status != EXPOS_MATCH:
            ...
            if all_raw:
                ...
                m_df, c_df = analyze_expos(pd.DataFrame(unique_raw), "bun/ji폴백", skip_dong_check=False)
                if not m_df.empty:
                    df_expos          = m_df
                    expos_status      = EXPOS_MATCH
                    df_expos_conflict = pd.DataFrame()
    ...
    for (js, jb, jp, jbun, jji) in floor_order:
        ...
        for p_gb in [jp] + [x for x in plat_cands if x != jp]:
            ...
            if not filtered.empty:
                ...
                filtered["_n"] = pd.to_numeric(
                    filtered["flrNo"] if "flrNo" in filtered.columns else pd.Series([None] * len(filtered)),
                    errors="coerce"
                ).fillna(-99)
    ...
    return (
        ...
        len(all_jibun), target_exact_jibun, expos_debug_log,
    )
...
def get_real_estate_data(sigungu_code, start_month, end_month, dong_name, prop_type, trans_type):
    ...
    if dong_name.strip() and "umdNm" in df.columns:
        df = df[df["umdNm"].astype(str).str.contains(dong_name.strip(), na=False)]
    ...
    df = df.rename(columns={
        ...
        "reqGbn": "거래유형", "dealingGbn": "거래유형",
        "cnclYmd": "계약취소일", "cdealDay": "계약취소일",
        "estbDvsnNm": "중개사소재지", "estateAgentSggNm": "중개사소재지",
        "buildingUse": "건물주용도", "buyerGbn": "매수자", "slerGbn": "매도자",
        "shareDealingType": "지분거래여부", "sggNm": "시군구",
    })

    if "법정동" in df.columns and "지번" in df.columns:
        df["소재지"] = df["법정동"].fillna("").astype(str) + " " + df["지번"].fillna("").astype(str).str.strip()
    ...
    if trans_type == "매매" and "거래금액" in df.columns:
        area_col = next(
            (c for c in ["전용면적", "건물면적", "연면적", "거래면적", "대지면적", "계약면적"] if c in df.columns),
            None
        )
        if area_col:
            def calc_pyeong(row):
                try:
                    ...
                    return ""
            df["평당가격"] = df.apply(calc_pyeong, axis=1)

    display_cols = [
        "계약일", "소재지", "건물유형", "용도지역", "건물주용도", "건물명", "건축년도",
        "대지면적", "건물면적", "연면적", "전용면적", "층",
        "거래금액", "평당가격", "보증금", "월세",
            ...
            if code:
                ...
                if not rdf.empty:
                    ...
                    with pd.ExcelWriter(buf, engine="xlsxwriter") as writer:
                        rdf.to_excel(writer, index=True, index_label="순번", sheet_name="실거래가")
                    st.download_button("📥 엑셀 다운로드", data=buf.getvalue(), file_name="실거래가.xlsx")
            else:
...
with tab2:
    ...
    st.caption("전유공용면적 충돌 탐지 포함 | 여러 필지에 걸친 단지 자동 수집")

    with st.form("form_bld"):
        c1, c2, c3 = st.columns([3, 1, 1])
        with c1:
            addr_in = st.text_input("지번 주소 (필수)", placeholder="예: 상도동 450")
        with c2:
            dong_in = st.text_input("동 (선택)", placeholder="예: 106")
        with c3:
            ho_in   = st.text_input("호수 (선택)", placeholder="예: 302")
