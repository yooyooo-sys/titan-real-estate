...
def get_sigungu_code(sigungu_name, dong_name):
    ...
    try:
        ...
        if data.get("StanReginCd"):
            ...
            active_regions = [r for r in rows if r["sido_cd"] != "" and r["sgg_cd"] != ""]
...
def get_real_estate_data(sigungu_code, start_month, end_month, dong_name, prop_type, trans_type):
    ...
    for i, target_month in enumerate(month_list):
        status_text.text(f"⏳ {target_month} 실거래가 데이터를 가져오는 중... ({i+1}/{len(month_list)})")
    ...
    try:
        ...
        if data.get("StanReginCd"):
            ...
            if active_regions:
                ...
                sgg_cd = region_cd[:5]
                bjdong_cd = region_cd[5:10]  # ✅ 수정: [5:] → [5:10] (정확히 5자리만 추출)
                return sgg_cd, bjdong_cd, active_regions[0]["locatadd_nm"]
...
def get_comprehensive_ledger(sgg_cd, bjdong_cd, plat_gb_cd, bun, ji, target_dong="", target_ho=""):
    ...
    # ✅ 수정 1: getBrBasisOulnInfo(기본개요) → getBrRecapTitleInfo(총괄표제부)
    ...
    # ✅ 수정 2: getBrExposInfo(존재X) → getBrExposPubuseAreaInfo(전유공용면적)
    ...
    def match_dong(t_dong, d_val, b_val):
        ...
        if t_clean == d_clean or f"{t_clean}동" in b_clean:
            return True
        t_nums = re.findall(r"\d+", t_str)
        if t_nums:
            t_num = t_nums[-1]
            short_num = str(int(t_num) % 100)
            if d_clean in [f"주{short_num}", str(short_num), f"주{t_num}", t_num]:
                return True
            if f"{t_num}동" in b_clean:
                return True
        return False
    ...
    def match_ho(h_target, h_val):
        ...
        if t_nums and h_nums and t_nums[-1] == h_nums[-1]:
            return True
        return False
    ...
    for p_gb in plat_candidates:
        ...
        for item in items:
            ...
            b_nm = item.get("bldNm", "")
            if pk:
                pk_map[pk] = {"dong": d_nm, "bld": b_nm}
            if target_dong and match_dong(target_dong, d_nm, b_nm):
                target_pks.add(pk)
        if target_pks:
            break

    # ✅ 수정 3: restore() 함수를 pk_map 정의 직후 상위 스코프로 이동
    # (기존에는 if target_ho: 블록 안에 있어 target_ho 없이 target_dong만 쓰면 NameError 발생)
    def restore(r, k):
        v = r.get(k)
        if not v or str(v).strip() in ["None", "nan", ""]:
            pk = r.get("mgmBldrgstPk")
            if pk and pk in pk_map:
                return pk_map[pk].get("dong" if k == "dongNm" else "bld")
        return v

    # 2️⃣ 총괄표제부 탐색 (✅ 수정된 URL_RECAP 사용)
    df_basis = pd.DataFrame()
    for p_gb in ["0", "2", "3", "1"]:
        basis_items = fetch_molit_api(URL_RECAP, sgg_cd, bjdong_cd, p_gb, bun, ji, status_text, f"[지번코드 {p_gb}] 단지개요 탐색", 2)
        valid_basis = [b for b in basis_items if float(str(b.get("totArea", "0")).replace(",", "") or 0) > 0.0]
        if valid_basis:
            df_basis = pd.DataFrame(valid_basis)
            break

    # 3️⃣ 전유공용면적 탐색 (✅ 수정된 URL_EXPOS 사용)
    ...
    if target_ho:
        for p_gb in plat_candidates:
            ...
            if target_dong and not matched_ho.empty:
                matched_ho = matched_ho[matched_ho.apply(
                    lambda r: r.get("mgmBldrgstPk") in target_pks or match_dong(target_dong, r.get("dongNm"), r.get("bldNm")), axis=1
                )]
    ...
    # 4️⃣ 층별개요 탐색 (✅ restore() 함수 이제 여기서도 정상 접근 가능)
    ...
    if target_dong:
        for p_gb in plat_candidates:
            ...
            if target_pks:
                ...
                temp_floor = temp_floor[temp_floor.apply(
        ...
        if not sigungu_name:
            ...
            if sigungu_code:
                ...
                if not real_data_df.empty:
                    ...
                    st.download_button("📥 엑셀 파일로 다운로드", data=excel_buffer.getvalue(), file_name="실거래가.xlsx")
            else:
        ...
        if not region_search_term:
            ...
            if sgg_cd and bjdong_cd:
                ...
                if df_title.empty and df_expos.empty:
                    st.warning("🚨 조회 결과가 없습니다. 지번이나 동/호수를 다시 한번 확인해주세요.")
                else:
                    ...
                    if ho_input and not df_expos.empty:
                        ...
                        for pk in unique_pks[:3]:
                            ...
                            with st.container(border=True):
                                ...
                                st.warning("🔒 **[데이터 한계 안내]** 소유자명은 개인정보보호로 제공되지 않으며, 일부 구축 건물의 경우 면적이 누락될 수 있습니다.")
                                j_str = "<span style='color:red;'>⚠️ API 전산 누락</span>" if is_missing_area else f"<span style='color:#0066cc; font-weight:bold;'>{j_area:,.2f} ㎡</span>"
                                g_str = "<span style='color:red;'>⚠️ 누락</span>" if is_missing_area else f"{g_area:,.2f} ㎡"
                                t_str = "<span style='color:#d93025; font-weight:bold;'>확인 불가</span>" if is_missing_area else f"<span style='color:#d93025; font-weight:bold;'>{t_area:,.2f} ㎡</span>"
                                st.markdown(f"""
                                | 구분 | 상세 내용 | 구분 | 상세 내용 |
                                |:---:|---|:---:|---|
