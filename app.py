            ...
            for (js, jb, jp, jbun, jji) in all_jibun:
                ...
                for p_gb in [jp] + [x for x in ["2", "3", "0", "1"] if x != jp]:
                    items, total_count = fetch_expos_by_dong_ho(
                        js, jb, p_gb, jbun, jji, dong_api, ho_api
                    )
                    expos_debug_log.append({
                        "방식":           "dongNm+hoNm 서버필터",
                        "파라미터":        f"dongNm={dong_api}, hoNm={ho_api}",
                        "지번":           f"bun={jbun} ji={jji} platGb={p_gb}",
                        "API_totalCount": total_count,
                        "실제수집건수":     len(items),
                        "hoNm샘플":       [x.get("hoNm", "") for x in items[:3]],
        ...
        # 2단계: hoNm 단독 서버 필터 (동 없는 경우)
            ...
            for (js, jb, jp, jbun, jji) in all_jibun:
                if expos_status == EXPOS_MATCH:
                    break
                for p_gb in [jp] + [x for x in ["2", "3", "0", "1"] if x != jp]:
                    items, total_count = fetch_expos_by_ho(js, jb, p_gb, jbun, jji, ho_api)
                    expos_debug_log.append({
                        "방식":           "hoNm 단독 서버필터",
                        "파라미터":        f"hoNm={ho_api}",
                        "지번":           f"bun={jbun} ji={jji} platGb={p_gb}",
                    ...
                    if items:
                        m_df, c_df = analyze_expos(pd.DataFrame(items), "hoNm단독서버필터")
                        if not m_df.empty:
                            df_expos     = m_df
                            expos_status = EXPOS_MATCH
                            break
