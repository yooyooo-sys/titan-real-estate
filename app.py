...
# ==========================================
# 🌟 [기능 2] 건축물대장 처리 함수 (V23: 정부24 완벽 복제 & 면적 스나이퍼)
# ==========================================
import re 
import pandas as pd
import requests
...
def fetch_all_data(base_url, sgg_cd, bjdong_cd, plat_gb_cd, bun, ji, status_text, desc):
    ...
    while page <= 50: 
        ...
        if status_text:
            status_text.info(f"⏳ {desc} 데이터 수집 중... ({page}페이지)")
...
def get_comprehensive_ledger(sgg_cd, bjdong_cd, plat_gb_cd, bun, ji, target_dong="", target_ho=""):
    ...
    URL_EXPOS = "http://apis.data.go.kr/1613000/BldRgstHubService/getBrExposPubuseAreaInfo" # ✅ 클로드가 찾은 올바른 API 유지
    ...
    for p_gb in plat_candidates:
        df_temp = fetch_all_data(URL_TITLE, sgg_cd, bjdong_cd, p_gb, bun, ji, status_text, f"표제부")
    ...
    # 3. 🌟 전유부 (면적) 탐색 - 클로드의 필터링 버그 완벽 수정본 🌟
    df_expos = pd.DataFrame()
    is_missing_area = False
    
    if target_ho:
        df_expos_raw = pd.DataFrame()
        # 모든 금고(지번코드)의 전유부 데이터를 싹 쓸어담습니다.
        for p_gb in plat_candidates:
            temp_df = fetch_all_data(URL_EXPOS, sgg_cd, bjdong_cd, p_gb, bun, ji, status_text, f"전유공용면적")
            if not temp_df.empty:
                df_expos_raw = pd.concat([df_expos_raw, temp_df], ignore_index=True)
                
        if not df_expos_raw.empty:
            # 1차 필터: 호수만 완벽하게 일치하는 녀석들 먼저 추출 (PK 조건 삭제!)
            matched_ho = df_expos_raw[df_expos_raw.apply(lambda r: match_ho(target_ho, r.get('hoNm')), axis=1)]
            
            if not matched_ho.empty:
                # 2차 필터: 동 필터링 시도
                if target_dong:
                    matched_dong = matched_ho[matched_ho.apply(lambda r: match_dong(target_dong, r.get('dongNm'), r.get('bldNm')), axis=1)]
                    if not matched_dong.empty:
                        df_expos = matched_dong # 완벽 매칭 성공
                    else:
                        df_expos = matched_ho # 동 이름 누락 시, 찾은 호수 데이터라도 강제 출력! (버그 방어막)
                else:
                    df_expos = matched_ho
    ...
    if target_dong:
        ...
        if not df_floor_raw.empty:
            ...
            if 'flrNo' in df_floor.columns:
                df_floor['flrNo_num'] = pd.to_numeric(df_floor['flrNo'], errors='coerce').fillna(-99)
...
# ----------------- [탭 2] 건축물대장 (정부24 양식) -----------------
with tab2:
    st.subheader("📋 건축물대장 통합 발급 화면 (정부24 서식)")
    st.info("💡 대단지 아파트는 **'동'**과 **'호수'**를 모두 입력하셔야 전용/공용면적이 정상 출력됩니다.")
    ...
    with st.form("bldrgst_form"):
        ...
        bld_submitted = st.form_submit_button("🔍 건축물대장 열람")
    ...
    if bld_submitted and address_input:
        ...
        if not region_search_term:
            ...
            if sgg_cd and bjdong_cd:
                st.success(f"✅ 주소 파싱 완료: {full_loc_name} (본번:{bun} 부번:{ji})")
                ...
                if df_title.empty and df_expos.empty and df_basis.empty:
                    ...
                    # 🚨 위반건축물 강력 경고 (KeyError 원천 차단)
                    is_violating = False
                    if not df_title.empty and 'violBldYn' in df_title.columns and '1' in df_title['violBldYn'].astype(str).values:
                        is_violating = True
                    if not df_basis.empty and 'violBldYn' in df_basis.columns and '1' in df_basis['violBldYn'].astype(str).values:
                        is_violating = True

                    if is_violating:
                        st.error("🚨 **[주의] 위반건축물(노란 딱지) 대장입니다!** 해당 건축물은 불법 증축 등의 위반 사항이 존재하므로, 중개 시 반드시 원본 서류를 확인해야 합니다.")

                    # 🟩 [섹션 1] 총괄표제부 (단지 정보)
                    if not df_basis.empty:
                        # 0면적 껍데기 거르기 로직
                        if 'totArea' in df_basis.columns:
                    ...
                    # 🟩 [섹션 3] 🌟핵심🌟 전유부 (전용/공용면적 추출)
                    if ho_input:
                        st.markdown(f"### 🏠 [전유부] 해당 호수 면적 상세")
                        if df_expos.empty:
                            st.warning("⚠️ 해당 호수의 면적 데이터를 국토부 API에서 찾지 못했습니다. (지번 또는 호수 번호를 다시 확인해주세요)")
                        else:
                            # PK를 기준으로 전유/공용 면적 합산
                            ...
                            for pk in unique_pks[:3]:
                                ...
                                with st.container(border=True):
                                    ...
                                    st.warning("🔒 오픈 API 특성상 소유자 성명 및 주민번호는 블라인드 처리됩니다.")
                    ...
                    # 🟩 [섹션 4] 층별 현황 (참고용으로 맨 아래 배치)
                    if not df_floor.empty:
                        with st.expander("🪜 [층별개요] 해당 동 층별 구조/면적 현황 (클릭하여 펼치기)"):
